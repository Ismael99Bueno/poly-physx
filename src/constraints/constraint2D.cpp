...
float constraint2D::compute_lambda(const std::tuple<glm::vec2, glm::vec2, glm::vec2> &dir_and_offsets) const
{
    const auto &[dir, offset1, offset2] = dir_and_offsets;
    const float cvel = constraint_velocity();

    const float cross1 = kit::cross2D(offset1, dir);
    const float cross2 = kit::cross2D(offset2, dir);

    const float inv_mass = m_body1->props().dynamic.inv_mass + m_body2->props().dynamic.inv_mass +
...
void constraint2D::apply_lambda(const float lambda, const std::tuple<glm::vec2, glm::vec2, glm::vec2> &dir_and_offsets)
{
    const auto &[dir, offset1, offset2] = dir_and_offsets;
    ...
    m_body1->ctr_proxy.angular_velocity += m_body1->props().dynamic.inv_inertia * kit::cross2D(offset1, imp1);
    m_body2->ctr_proxy.angular_velocity += m_body2->props().dynamic.inv_inertia * kit::cross2D(offset2, imp2);

    const glm::vec2 f1 = imp1 / world.integrator.ts.value;
    const glm::vec2 f2 = imp2 / world.integrator.ts.value;
    const float torque1 = kit::cross2D(offset1, f1);
    const float torque2 = kit::cross2D(offset2, f2);
...
{
    const auto dir_offsets = dir_and_offsets();
    const float lambda = compute_lambda(dir_offsets);
    ...
    if (!kit::approaches_zero(delta_lambda))
        apply_lambda(delta_lambda, dir_offsets);
}
...
void constraint2D::solve_unclamped()
{
    ...
    const float lambda = compute_lambda(dir_offsets);
    m_cumlambda += lambda;
    apply_lambda(lambda, dir_offsets);
}
...
void constraint2D::warmup()
{
    ...
    apply_lambda(m_cumlambda, dir_and_offsets());
}
