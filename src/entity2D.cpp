...
namespace ppx
{
entity2D::entity2D(const glm::vec2 &pos, const glm::vec2 &vel, const float angpos, const float angvel, const float mass,
...
entity2D::entity2D(const float radius, const glm::vec2 &pos, const glm::vec2 &vel, const float angpos,
                   const float angvel, const float mass, const float charge, const bool kinematic)
...
entity2D::entity2D(const specs &spc)
...
void entity2D::add_force(const glm::vec2 &force)
{
    m_added_force += force;
}
void entity2D::add_torque(const float torque)
{
    m_added_torque += torque;
}

const glm::vec2 &entity2D::added_force() const
{
    return m_added_force;
}
float entity2D::added_torque() const
{
...
template <typename T> const T &entity2D::shape() const
{
...
template <typename T> const T *entity2D::shape_if() const
{
...
float entity2D::inverse_inertia() const
{
    return m_inv_inertia;
}

bool entity2D::kinematic() const
{
    return m_kinematic;
}
void entity2D::kinematic(const bool kinematic)
{
    m_kinematic = kinematic;
}

void entity2D::translate(const glm::vec2 &dpos)
{
    get_shape().translate(dpos);
}
void entity2D::rotate(const float dangle)
{
    get_shape().rotate(dangle);
}

const entity_events &entity2D::events() const
{
    return m_events;
}
entity_events &entity2D::events()
{
    return m_events;
}

const glm::vec2 &entity2D::pos() const
{
    return shape().centroid();
}
const glm::vec2 &entity2D::vel() const
{
    return m_vel;
}
const glm::vec2 entity2D::vel_at(const glm::vec2 &at) const
{
    return m_vel + m_angvel * glm::vec2(-at.y, at.x);
}

float entity2D::angpos() const
{
    return shape().rotation();
}
float entity2D::angvel() const
{
    return m_angvel;
}

float entity2D::mass() const
{
    return m_mass;
}
float entity2D::inverse_mass() const
{
    return m_inv_mass;
}
float entity2D::charge() const
{
    return m_charge;
}
...
void entity2D::vel(const glm::vec2 &vel)
{
    m_vel = vel;
}
...
void entity2D::angvel(const float angvel)
{
    m_angvel = angvel;
}
...
void entity2D::charge(const float charge)
{
    m_charge = charge;
}
...
entity2D::specs entity2D::specs::from_entity(const entity2D &e)
{
    if (const auto *poly = e.shape_if<geo::polygon>())
        return {e.pos(),    e.vel(),        e.angpos(), e.angvel(),    e.mass(),
                e.charge(), poly->locals(), 0.f,        e.kinematic(), e.type()};
    return {e.pos(),       e.vel(), e.angpos(), e.angvel(), e.mass(), e.charge(), {}, e.shape<geo::circle>().radius(),
            e.kinematic(), e.type()};
}
