...
collision_manager2D::collision_manager2D(world2D &world) : worldref2D(world)
{
    ...
    m_custom_detection->set_cc_manifold_algorithm<radius_distance_manifold2D>();
    m_custom_detection->set_cp_manifold_algorithm<mtv_support_manifold2D>();
    m_custom_detection->set_pp_manifold_algorithm<clipping_algorithm_manifold2D>();
}

const collision2D &collision_manager2D::operator[](const std::size_t index) const
{
    KIT_ASSERT_ERROR(index < m_custom_detection->collisions().size(), "Index exceeds container size: {0}", index)
    return m_custom_detection->collisions()[index];
}

void collision_manager2D::solve()
{
    const auto &collisions = m_custom_detection->detect_collisions_cached();
    m_custom_resolution->solve(collisions);
}

std::size_t collision_manager2D::size() const
{
    return m_custom_detection->collisions().size();
}
bool collision_manager2D::empty() const
{
    return m_custom_detection->collisions().empty();
}
