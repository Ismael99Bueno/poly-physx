...
float rotor_joint2D::constraint_velocity() const
{
    ...
    if (props.spin_indefinitely)
        return dw;
    const float da =
        m_body2->ctr_state.centroid.rotation() - m_body1->ctr_state.centroid.rotation() - props.target_offset;
    return dw + props.correction_factor * da / world.rk_substep_timestep();
}
