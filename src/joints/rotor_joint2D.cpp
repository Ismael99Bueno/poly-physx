...
void rotor_joint2D::update_constraint_data()
{
    ...
    if (props.spin_indefinitely)
        return;
    float da = m_body2->meta.ctr.state.centroid.rotation() - m_body1->meta.ctr.state.centroid.rotation();
    da -= glm::round(da / glm::two_pi<float>()) * glm::two_pi<float>();
    m_relangle = da;

    if (da < props.min_angle)
        da =
            m_body2->meta.ctr.state.centroid.rotation() - m_body1->meta.ctr.state.centroid.rotation() - props.min_angle;
    else if (da > props.max_angle)
        da =
            m_body2->meta.ctr.state.centroid.rotation() - m_body1->meta.ctr.state.centroid.rotation() - props.max_angle;
    else
    {
        m_correction = 0.f;
        m_legal_angle = true;
