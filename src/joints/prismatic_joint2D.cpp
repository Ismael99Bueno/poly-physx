...
prismatic_joint2D::prismatic_joint2D(world2D &world, const specs &spc)
    : pvconstraint2D<1, 1>(world, spc, spc.ganchor1, spc.ganchor2), props(spc.props),
      m_target_relangle(m_body2->proxy.ctr_state.centroid.rotation() - m_body1->proxy.ctr_state.centroid.rotation())
...
glm::vec2 prismatic_joint2D::constraint_position() const
{
    ...
    const float da =
        m_body2->proxy.ctr_state.centroid.rotation() - m_body1->proxy.ctr_state.centroid.rotation() - m_target_relangle;
    return {dp, da};
}
glm::vec2 prismatic_joint2D::constraint_velocity() const
{
    const float dv = glm::dot(m_dir, m_body2->proxy.ctr_state.velocity_at_centroid_offset(m_offset2) -
