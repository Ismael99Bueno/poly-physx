...
ball_joint2D::ball_joint2D(world2D &world, const specs &spc) : pvconstraint2D<0, 1>{world, spc}, props{spc.props}
{
    if (spc.deduce_angle)
    {
        m_relangle = m_body2->meta.ctr.state.centroid.rotation() - m_body1->meta.ctr.state.centroid.rotation();
...
{
    return m_body2->meta.ctr.state.angular_velocity - m_body1->meta.ctr.state.angular_velocity;
}
